
class Tree {
    private Node root;
    // Левый поворот
    private Node leftSwap(Node node) {
        Node leftChild = node.leftChild;
        Node betweenChild = leftChild.rightChild;
        leftChild.rightChild = node;
        node.leftChild = betweenChild;
        leftChild.color = node.color;
        node.color = true;
        return leftChild;
    }
    //Правый поворот
    private Node rightSwap(Node node) {
        Node rightChild = node.rightChild;
        Node betweenChild = rightChild.leftChild;
        rightChild.leftChild = node;
        node.rightChild = betweenChild;
        rightChild.color = node.color;
        node.color = true;
        return rightChild;
    }
    // Замена цвета, если оба дочерних элемента красные
    private void colorSwap(Node node) {
        node.rightChild.color = false;
        node.leftChild.color = false;
        node.color = true;
    }
    //Добавляем новый элемент
    public void add(int key) {
        if (root != null) {
            boolean result = addNode(root, key);
            root = rebalanced(root);
            root.color = false;
        } else {
            root = new Node();
            root.color = false;
            root.key = key;
        }
    }

    private boolean addNode(Node node, int key) {
        if (key == node.key) {
            return false;
        } else {
            if (key < node.key) {
                if (node.leftChild != null) {
                    boolean result = addNode(node.leftChild, key);
                    node.leftChild = rebalanced(node.leftChild);
                    return result;
                } else {
                    node.leftChild = new Node();
                    node.leftChild.color = true;
                    node.leftChild.key = key;
                    return true;
                }
            } else {
                if (node.rightChild != null) {
                    boolean result = addNode(node.rightChild, key);
                    node.rightChild = rebalanced(node.rightChild);
                    return result;
                } else {
                    node.rightChild = new Node();
                    node.rightChild.color = true;
                    node.rightChild.key = key;
                    return true;
                }

            }
        }
    }
    //Ребалансировка
    private Node rebalanced(Node node) {
        Node result = node;
        boolean needRebalance;
        do {
            needRebalance = false;
            if (result.rightChild != null && result.rightChild.color == true
                    && (result.leftChild == null || result.leftChild.color == false)) {
                needRebalance = true;
                result = rightSwap(result);
            }
            if (result.leftChild != null && result.leftChild.color == true && result.leftChild.leftChild != null
                    && result.leftChild.leftChild.color == true) {
                needRebalance = true;
                result = leftSwap(result);
            }
            if (result.leftChild != null && result.leftChild.color == true && result.rightChild != null
                    && result.rightChild.color == true) {
                needRebalance = true;
                colorSwap(result);
            }
        } while (needRebalance);
        return result;
    }


    private static class Node {
        int key;
        Node leftChild;
        Node rightChild;
        // красный(true), черный(false)
        boolean color;
    }
}